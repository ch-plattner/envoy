...
public:
  ...
  waitForImpl(Thread::MutexBasicLockable& mutex, Thread::CondVar& condvar, const Duration& duration,
              bool always_sleep) noexcept ABSL_EXCLUSIVE_LOCKS_REQUIRED(mutex) PURE;

  template <class D>
  Thread::CondVar::WaitStatus waitFor(Thread::MutexBasicLockable& mutex, Thread::CondVar& condvar,
                                      const D& duration, bool always_sleep = false) noexcept
      ABSL_EXCLUSIVE_LOCKS_REQUIRED(mutex) {
    return waitForImpl(mutex, condvar, std::chrono::duration_cast<Duration>(duration),
                       always_sleep);
  }
};
...
public:
  ...
  Thread::CondVar::WaitStatus waitForImpl(Thread::MutexBasicLockable& mutex,
    ...
    return timeSystem().waitForImpl(mutex, condvar, duration, always_sleep);
  }
